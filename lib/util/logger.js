...
var fs = require('fs');
if (process.env.NIGHTWATCH_LOG_PATH) {
  var placeForLog = process.env.NIGHTWATCH_LOG_PATH;
  var bWrite = true;
  if (!fs.existsSync(placeForLog)) {
    fs.mkdirSync(placeForLog)
  }
  placeForLog = `${placeForLog}/nightwatch.log`;
}
else {
  bWrite = false
}
...
function logObject(obj) {
  if (bWrite) {
    fs.appendFileSync(placeForLog, util.inspect(obj, {
      showHidden: false,
      depth: 3,
...
function logMessage(type, message, args) {
  ...
  if (bWrite) {
    fs.appendFileSync(placeForLog, "\n");
    fs.appendFileSync(placeForLog, messageStr);
  }
  else {
    ...
    args.forEach(function(item) {
      if (Object.prototype.toString.call(item) === '[object Object]' && Object.keys(item).length > 0) {
        if (inlineArgs.length) {
          if (bWrite) {
            fs.appendFileSync(placeForLog, inlineArgs);
          }
          else {
